...
def run():
    ...
    for row in csv_reader:
        if line_count != 0:
            ...
            release_date = datetime.strptime(release_date, '%m/%d/%Y') if release_date else datetime.strptime('01/01/2010', '%m/%d/%Y')
            revenue = row['revenue']
            ...
            movie = Movie(title=title, tagline=tangline, overview=overview, poster_url=poster_path,
                          ...
                          release_date=release_date, revenue=revenue, genres=genres, duration=duration)
